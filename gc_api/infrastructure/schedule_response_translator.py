...
class ScheduleResponseTranslator:
    ...
    def convert(
        self, data: list[dict], start_date: Date, end_date: Date
        ...
        filtered_data = self._filter_valid_datetime_range(
            filtered_data, start_datetime, end_datetime
        )
    ...
    def _convert_schedule(self, schedule: dict) -> ScheduleResponse:
        ...
        description_text: str = schedule.get("description", "")
    ...
    def _filter_valid_datetime_range(
        self, data: list[dict], start_datetime: DateTime, end_datetime: DateTime
    ) -> list[dict]:
        result = []
        for schedule in data:
            # 日時のバリデーション
            ...
            if not (
                start_datetime.timestamp() <= start.timestamp()
                and end.timestamp() <= end_datetime.timestamp()
            ):
                continue
